...
public final class Sistema {
    ...
    public void iniciarSession(final int numeroDeSocio, final String contrasenia) {
        ...
        //// el numero de socio no puede exceder el limite del arreglo
        if (numeroDeSocio > socios.length) {
            throw new IllegalArgumentException("El numero de socio no es valido!");
        }

        //// hacemos esto para poder utilizar variables de la clase Socio
        Socio socioprueba = socios[numeroDeSocio - 1];


        if (!contrasenia.equals(socioprueba.getContrasenia())) {
            throw new IllegalArgumentException("La contraseña no es valida!");

        }


        // Inicia la sesion del socio
        this.socio = socios[numeroDeSocio - 1];

        StdOut.println(" inicio de sesion correcto");
    ...
    public void realizarPrestamoLibro(final String isbn) throws IOException {
        ...
        this.guardarInformacion();
        StdOut.println("Informacion actualizada con exito");
    ...
    /**
     * Metodo que ingresa la nueva contraseña
     *
     * @return la nueva contraseña del socio.
     */
    public String obtenernuevacontrasenia() {
        StdOut.println("Ingrese la nueva contraneña");
    ...
    /**
     * Metodo que ingresa el nuevo correo
     *
     * @return el nuevo correo del socio.
     */
    public String obtenerNuevoCorreo() {
        StdOut.println("Ingrese el nuevo correo eléctronico: ");
    ...
    /**
     * Metodo que ingresa el nuevo apellido al sistema
     *
     * @return el apellido modificado
     */
    public String obtenernuevoApellido() {
    ...
    /**
     * Metodo que ingresa el nuevo nombre al sistema
     *
    ...
    public double calificacionLibro(String isbn) {

        for (Libro libro : this.libros) {
            if (libro.getIsbn().equals(isbn)) {

                StdOut.println("Ingrese la calificacion que le desea colocar al libro con ISBN:" + isbn);
                double calificacion = StdIn.readDouble();
                if (calificacion > 5 || calificacion < 0) {
                    StdOut.println("Error el valor de la calificacion excede los limites");
                }
                //Variable para poder dividir y calcular el promedio
                int auxPromedio = 1;

                double calificacionAux = 0;
                calificacionAux += calificacion;

                calificacion = calificacion / auxPromedio;
